#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: AthenaInventoryDragVisual

#include "Basic.hpp"

#include "AthenaInventoryDragVisual_classes.hpp"
#include "AthenaInventoryDragVisual_parameters.hpp"


namespace SDK
{

// Function AthenaInventoryDragVisual.AthenaInventoryDragVisual_C.ShowDropIcon
// (Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// EAthenaDragDropAction                   Drop_Action                                            (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UAthenaInventoryDragVisual_C::ShowDropIcon(EAthenaDragDropAction Drop_Action)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("AthenaInventoryDragVisual_C", "ShowDropIcon");

	Params::AthenaInventoryDragVisual_C_ShowDropIcon Parms{};

	Parms.Drop_Action = Drop_Action;

	UObject::ProcessEvent(Func, &Parms);
}


// Function AthenaInventoryDragVisual.AthenaInventoryDragVisual_C.ExecuteUbergraph_AthenaInventoryDragVisual
// (Final, UbergraphFunction)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UAthenaInventoryDragVisual_C::ExecuteUbergraph_AthenaInventoryDragVisual(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("AthenaInventoryDragVisual_C", "ExecuteUbergraph_AthenaInventoryDragVisual");

	Params::AthenaInventoryDragVisual_C_ExecuteUbergraph_AthenaInventoryDragVisual Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function AthenaInventoryDragVisual.AthenaInventoryDragVisual_C.Construct
// (BlueprintCosmetic, Event, Public, BlueprintEvent)

void UAthenaInventoryDragVisual_C::Construct()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("AthenaInventoryDragVisual_C", "Construct");

	UObject::ProcessEvent(Func, nullptr);
}


// Function AthenaInventoryDragVisual.AthenaInventoryDragVisual_C.CanSwap
// (Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent, BlueprintPure)
// Parameters:
// bool*                                   CanSwap_0                                              (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UAthenaInventoryDragVisual_C::CanSwap(bool* CanSwap_0)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("AthenaInventoryDragVisual_C", "CanSwap");

	Params::AthenaInventoryDragVisual_C_CanSwap Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (CanSwap_0 != nullptr)
		*CanSwap_0 = Parms.CanSwap_0;
}

}

